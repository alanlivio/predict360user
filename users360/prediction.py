...
def get_train_test_split(df_trajects: pd.DataFrame, train_entropy: str,
                         test_entropy: str,
  ...
  if train_entropy.endswith('_users') and train_entropy != 'all':
    ...
    x_train, _ = train_test_split(
        df_trajects[df_trajects['traject_entropy_class'] == train_entropy],
        **args)
  ...
  if test_entropy.endswith('_users') and test_entropy != 'all':
    _, test_entropy = train_test_split(
        df_trajects[df_trajects['user_entropy_class'] ==
                    test_entropy.removesuffix('_users')], **args)
  elif test_entropy != 'all':
    _, x_test = train_test_split(
        df_trajects[df_trajects['traject_entropy_class'] == test_entropy],
        **args)

  return x_train, x_test


def show_train_test_split(df_trajects: pd.DataFrame, train_entropy: str,
                          test_entropy: str, perc_test: float) -> None:
  x_train, x_test = get_train_test_split(df_trajects, train_entropy,
                                         test_entropy, perc_test)
