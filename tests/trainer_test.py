...
class Test(unittest.TestCase):
  ...
  def test_init(self) -> None:
    trn = Trainer()
    self.assertEquals(trn.model_fullname, 'pos_only')
    self.assertEquals(trn.model_dir, join(config.DATADIR, trn.model_fullname))
    sefl.assertEquals(trn.using_auto, False)
    trn = Trainer(dataset_name='david')
    self.assertEquals(trn.model_fullname, 'pos_only,david,,')
    sefl.assertEquals(trn.using_auto, False)
  ...
  def test_train_test_split(self) -> None:
    ...
    self.assertEquals(unique_s, set(['low', 'medium', 'hight']))
    ...
    unique_s = set(x_train['actS_c'].unique())
    self.assertEquals(unique_s, set(['low']))
    unique_s = set(x_test['actS_c'].unique())
    self.assertEquals(unique_s, set(['low']))
    ...
    unique_s = set(x_train['actS_c'].unique())
    self.assertEquals(unique_s, set(['medium']))
    unique_s = set(x_test['actS_c'].unique())
    self.assertEquals(unique_s, set(['medium']))
    ...
    unique_s = set(x_train['actS_c'].unique())
    self.assertEquals(unique_s, set(['hight']))
    unique_s = set(x_test['actS_c'].unique())
    self.assertEquals(unique_s, set(['hight']))
  ...
  def test_partition(self) -> None:
    trn = Trainer()
    trn.partition()
    self.assertEquals((count_traject_entropy_classes(trn.x_train), (10301, 6125, 3133, 1043)))
    self.assertEquals((count_traject_entropy_classes(trn.x_test), (2576, 1510, 814, 252)))
    trn = Trainer(train_entropy='low')
    trn.partition()
    self.assertEquals((count_traject_entropy_classes(trn.x_train), (6108, 6108, 0, 0)))
    self.assertEquals((count_traject_entropy_classes(trn.x_test), (1527, 1527, 0, 0)))
    trn = Trainer(train_entropy='medium')
    trn.partition()
    self.assertEquals((count_traject_entropy_classes(trn.x_train), (3157, 0, 3157, 0)))
    self.assertEquals((count_traject_entropy_classes(trn.x_test), (790, 0, 790, 0)))
    trn = Trainer(train_entropy='hight')
    trn.partition()
    self.assertEquals((count_traject_entropy_classes(trn.x_train), (1036, 0, 0, 1036)))
    self.assertEquals((count_traject_entropy_classes(trn.x_test), (259, 0, 0, 259)))
    trn = Trainer(train_entropy='nohight')
    trn.partition()
    self.assertEquals((count_traject_entropy_classes(trn.x_train), (9265, 6089, 3176, 0)))
    self.assertEquals((count_traject_entropy_classes(trn.x_test), (2317, 1546, 771, 0)))