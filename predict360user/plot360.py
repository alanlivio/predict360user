...
from predict360user.tileset import TILESET_DEFAULT, TileSet, TileSetVoro, tile_points
...
class Plot360:
    ...
    def add_traces(
        ...
        for index in range(n - 1):
            ...
            edge = go.Scatter3d(
                ...
                hoverinfo="text",
                mode="lines",
                line={"width": 7, "color": colors[index]},
            )
            self.data.append(edge)
    ...
    def add_predictions(self, predictions: dict) -> None:
        ...
        for i, (_, traces) in enumerate(predictions.items()):
            self.add_traces(
                traces, self.prediction_start_c, self.prediction_end_c, visible=False
            )
            step = dict(
                method="update",
        ...
        self.sliders = [
            dict(active=0, currentvalue={"prefix": "Predition at: "}, steps=steps)
        ]
    ...
    def show_fov(self, trace) -> None:
        ...
        title = (
            f"trace_[{trace[0]:.2},{trace[1]:.2},{trace[2]:.2}]_{self.tileset.prefix}"
        )
