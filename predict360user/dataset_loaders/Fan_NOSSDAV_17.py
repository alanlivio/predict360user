    ...
    for root, directories, files in os.walk(join(ROOT_FOLDER, "sensory/orientation")):
        for enum_trace, filename in enumerate(files):
            ...
            for pos, frame_id in zip(positions, frame_ids):
                samples.append(
                    {"sec": frame_id * video_rate, "yaw": pos[0], "pitch": pos[1]}
                )
...
def from_position_to_tile_probability_cartesian(pos) -> np.ndarray:
    yaw_grid, pitch_grid = np.meshgrid(
        np.linspace(0, 1, NUM_TILES_WIDTH, endpoint=False),
    ...
    great_circle_distance = np.arccos(
        np.maximum(
            np.minimum(x_grid * pos[0] + y_grid * pos[1] + z_grid * pos[2], 1.0), -1.0
        )
    )
    binary_orth = np.where(
        great_circle_distance < (((FOV_SIZE / 2.0) / 180.0) * np.pi), 1, 0
    )
    return binary_orth
    ...
    for enum_user, user in enumerate(original_dataset.keys()):
        for enum_video, video in enumerate(original_dataset[user].keys()):
            ...
            for sample_id, sample in enumerate(original_dataset[user][video]):
                ...
                sample_new = eulerian_to_cartesian(sample_yaw, sample_pitch)
                gen_tile_prob_cartesian = from_position_to_tile_probability_cartesian(
                    sample_new
                )
...
def create_and_store_sampled_dataset() -> None:
    ...
    filtered_sampled_dataset = create_sampled_dataset(
        filtered_original_dataset, rate=SAMPLING_RATE
    )
...
def create_saliency_maps() -> None:
    ...
    videos = [
        d.split("_")[0]
        for d in os.listdir(folder_sal_map)
        if isdir(join(folder_sal_map, d))
    ]
    for video in videos:
        sal_per_vid = {}
