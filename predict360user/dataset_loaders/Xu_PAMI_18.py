...
def get_dot_mat_data():
    ...
    for video in mat.keys():
        ...
        for user in range(int(mat[video].shape[1] / 2)):
            ...
            lat = latitudes[:, user]
            dataset[user][video] = np.concatenate(
                (long[:, np.newaxis], lat[:, np.newaxis]), axis=1
            )
    return dataset
...
def compare_dot_mat_dataset_and_original_dataset(dot_mat_dataset, original_dataset):
    ...
    for i, user in enumerate(users):
        for video in dot_mat_dataset[i].keys():
            for sample_dot, sample_orig in zip(
                dot_mat_dataset[i][video], original_dataset[user][video]
            ):
                if np.isnan(sample_dot[0]) or np.isnan(sample_dot[1]):
                    print(
                        "Nan sample",
                        sample_dot,
                        "in sample_dot from video",
                        video,
                        "user",
                        user,
                    )
                ...
                if np.isnan(sample_orig[0]) or np.isnan(sample_orig[1]):
                    print(
                        "Nan sample",
                        sample_orig,
                        "in sample_orig from video",
                        video,
                        "user",
                        user,
                    )
                    flag_nan_in_orig = True
                if not np.isclose(sample_dot[0], sample_orig[0]) and np.isclose(
                    sample_dot[1], sample_orig[1]
                ):
    ...
    if flag_more_vids_in_orig:
        print(
            "There are some videos in the original dataset that are not in the .mat dataset"
        )
    if flag_more_vids_in_dot_mat:
        print(
            "There are some videos in the .mat dataset that are not in the original dataset"
        )
    if flag_nan_in_orig:
...
def store_dataset(xyz_dataset) -> None:
...
def load_sampled_dataset():
    ...
    for video in list_of_videos:
        for user in [
            o
            for o in os.listdir(os.path.join(OUTPUT_FOLDER, video))
