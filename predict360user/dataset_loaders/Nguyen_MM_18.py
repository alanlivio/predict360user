...
OUTPUT_FOLDER = join(config.DEFAULT_SAVEDIR, 'Nguyen_MM_18/sampled_dataset')
OUTPUT_SALIENCY_FOLDER = join(config.DEFAULT_SAVEDIR, 'Nguyen_MM_18/extract_saliency/saliency')
OUTPUT_TRUE_SALIENCY_FOLDER = join(config.DEFAULT_SAVEDIR, 'Nguyen_MM_18/true_saliency')
...
def get_original_dataset() -> dict:
  ...
  for video_id, video in enumerate(salient_ds_dict['360net'].keys()):
    ...
    for user_id in range(len(salient_ds_dict['360net'][video]['headpos'])):
      print('get head positions from original dataset', 'video', video_id, '/',
            len(salient_ds_dict['360net'].keys()), 'user', user_id, '/',
...
def create_saliency_maps() -> None:
  ...
  for video in salient_ds_dict['360net'].keys():
    ...
    time_stamps_by_rate = np.arange(time_stamps[id_sort_tstamps[0]],
...
def create_and_store_true_saliency(sampled_dataset) -> None:
  ...
  users_per_video = {}
  for video in videos:
    users_per_video[video] = [user for user in os.listdir(os.path.join(OUTPUT_FOLDER, video))]
