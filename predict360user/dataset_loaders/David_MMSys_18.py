        ...
        for enum_trace, filename in enumerate(files):
            ...
            for user_id in range(num_users):
                ...
                if user not in dataset.keys():
                    dataset[user] = {}
                positions = positions_all_users[
                    user_id
                    * NUM_SAMPLES_PER_USER : (user_id + 1)
                    * (NUM_SAMPLES_PER_USER)
                ]
                time_stamps = time_stamps_all_users[
                    user_id
                    * NUM_SAMPLES_PER_USER : (user_id + 1)
                    * (NUM_SAMPLES_PER_USER)
                ]
                ...
                for pos, t_stamp in zip(positions, time_stamps):
                    samples.append(
                        {"sec": t_stamp / 1000.0, "yaw": pos[0], "pitch": pos[1]}
                    )
